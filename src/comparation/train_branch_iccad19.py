# Geng2020 训练，Branch 1 & 2, for ICCAD'19 dataset
# train_branch_iccad19.py
...
class ICCAD19DatasetBr1(Dataset):
    def __init__(self, root_dir, transform=None, isTrain=True, training_set_proportion=0.7):
        ...
        print(len(self.hotspot_images), len(self.non_hotspot_images))
        ...
        if isTrain:
            self.hotspot_images = self.hotspot_images[:min(len(self.hotspot_images), 9600)]
            self.non_hotspot_images = self.non_hotspot_images[:min(len(self.non_hotspot_images), 22400)]
        else:
            self.hotspot_images = self.hotspot_images[:min(len(self.hotspot_images), 9600)]
...
class ICCAD19DatasetBr2(Dataset):
    def __init__(self, root_dir, transform=None, isTrain=True, training_set_proportion=0.7):
        """
        初始化数据集，加载图片路径和标签
        :param root_dir: 数据集根目录
...
def Stage1Debug(is_branch2 = False):    # 若为 Branch 1，False；否则为 True。
    ...
    if is_branch2 == False: # Branch 1
        ds_train = ICCAD19DatasetBr1(root_dir=train_dir, transform=transform_img)
        ...
        ds_train = ICCAD19DatasetBr2(root_dir=train_dir, transform=transform_img)
...
if __name__ == '__main__':
    ### -------------------- 0. 超参数 -------------------- ###
    # TODO 所有的超参数在这里！
    batch_sizes = [64]  # batch 太大会出现显存溢出问题。
    epochs_br1 = 60
    epochs_br2 = 20
    ...
    # 加载训练和测试数据集，假设以 iccad1 作为例子。
    train_dir = "/ai/edallx/Graduate_Project_2025/iccad19/iccad-Geng2020/"  # 修改为你的训练数据集路径。这里无需修改，Windows 和 Linux 都可以用它。
    test_dir = "/ai/edallx/Graduate_Project_2025/iccad19/iccad-Geng2020/"  # 修改为你的测试数据集路径
    dirs = ['iccad6']
    # dirs = ['']
    ...
    for eachlr in lr_rates_br1:     # 遍历五个 learning rate
        for eachbs in batch_sizes:  # 遍历所有 batch sizes
            for dis in dirs:    # 遍历五个不同的子目录
                ...
                print('\n' + train_dir + dis + '/train')
                ds_trainbr1 = ICCAD19DatasetBr1(root_dir=train_dir + dis + '/train', transform=transform_img, isTrain=True)    # TODO: 小心！
                print('\n' + test_dir + dis + '/test')
                ds_testbr1 = ICCAD19DatasetBr1(root_dir=test_dir + dis + '/test', transform=transform_img, isTrain=False)      # TODO: 小心！
                # 注意这里的 batch size！
                ...
                ds_trainbr2 = ICCAD19DatasetBr2(root_dir=train_dir + dis + '/train', transform=transform_img, isTrain=True)        # TODO: 小心！
                ds_testbr2 = ICCAD19DatasetBr2(root_dir=test_dir + dis + '/test', transform=transform_img, isTrain=False)          # TODO: 小心！
